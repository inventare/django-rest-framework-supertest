...
def color(fake, hue=None, luminosity=None, color_format="hex"):
    ...
    it to the desired color_format. The argument hue
    controls the H value according to the following rules:

    - If the value is a number from 0 to 360,
      it will serve as the H value of the generated color.
    ...
    - If the value is a valid string, the color’s H value will
      be randomly selected from the H range corresponding to
      the supplied string. Valid values are 'monochrome',
...
def unique_color(fake, hue=None, luminosity=None, color_format="hex"):
    """
    ...
    represented. Valid values are 'hsv', 'hsl', 'rgb', or 'hex' (default).
    """
    return unique(fake, color, hue=hue, luminosity=luminosity, color_format=color_format)

def color_name(fake):
    """Generate a color name."""
    return fake.color_name()

def unique_color_name(fake):
